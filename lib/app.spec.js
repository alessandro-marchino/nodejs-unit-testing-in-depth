const { expect, use } = require('chai');
const { createSandbox } = require('sinon');
const { default: chaiAsPromised } = require('chai-as-promised');
const { default: sinonChai } = require('sinon-chai');
const rewire = require('rewire');
const request = require('supertest');

let app = rewire('./app');
const users = require('./users');
const auth = require('./auth');
const { response } = require('./app');

use(chaiAsPromised);
use(sinonChai);

describe('app', () => {
  const sandbox = createSandbox();
  let handleErrorStub;

  beforeEach(() => {
    handleErrorStub = sandbox.stub();
  })

  afterEach(() => {
    app = rewire('./app');
    sandbox.restore();
  });

  context('GET /', () => {
    it('should get /', done => {
      request(app)
        .get('/')
        .expect(200)
        .end((err, response) => {
          expect(err).not.to.exist;
          expect(response.body).to.have.property('name').to.equal('Foo Fooing Bar');
          done(err);
        })
    })
  });

  context('POST /user', () => {
    let createUserStub;
    beforeEach(() => {
      createUserStub = sandbox.stub(users, 'createUser');

    })

    it('should call user.create', done => {
      createUserStub.resolves({ name: 'foo' });
      request(app)
        .post('/user')
        .send({ name: 'fake' })
        .expect(200)
        .end((err, response) => {
          expect(err).not.to.exist;
          expect(response.body).to.have.property('name').to.equal('foo');
          expect(createUserStub).to.have.been.calledOnce;
          done(err);
        })
    });

    it('should call handleError on error', done => {
      createUserStub.rejects(new Error('fake_error'));
      handleErrorStub.callsFake((res, err) => res.status(400).json({ error: 'fake' }));
      app.__set__('handleError', handleErrorStub)

      request(app)
        .post('/user')
        .send({ name: 'fake' })
        .expect(400)
        .end((err, response) => {
          expect(err).not.to.exist;
          expect(response.body).to.have.property('error').to.equal('fake');
          expect(createUserStub).to.have.been.calledOnce;
          expect(handleErrorStub).to.have.been.calledOnce;
          done(err);
        })
    });
  });

  context('DELETE /user/:id', () => {
    let isAuthorizedStub;
    let deleteUserStub;
    beforeEach(() => {
      isAuthorizedStub = sandbox.stub(auth, 'isAuthorized').callsFake((req, res, next) => next());
      app = rewire('./app');
      deleteUserStub = sandbox.stub(users, 'deleteUser');
    });

    it('should call auth check function and users.deleteUSer on success', done => {
      deleteUserStub.resolves('fake_delete');

      request(app)
        .delete('/user/123')
        .expect(200)
        .end((err, response) => {
          expect(err).not.to.exist;
          expect(response.body).to.equal('fake_delete');
          expect(isAuthorizedStub).to.have.been.calledOnce;
          expect(deleteUserStub).to.have.been.calledWith({ id: '123', name: 'foo' });
          done(err);
        })
    })
  });

  context('handleError', () => {
    let jsonStub;
    let statusStub;
    let handleError;

    beforeEach(() => {
      jsonStub = sandbox.stub();
      statusStub = sandbox.stub().returns({ json: jsonStub });
      handleError = app.__get__('handleError');
      res = { status: statusStub };
    });

    it('should check error instance and format message', () => {
      handleError({ status: statusStub }, new Error('fake'));
      expect(statusStub).to.have.been.calledWith(400);
      expect(jsonStub).to.have.been.calledWith({ error: 'fake' });
    });

    it('should return object without changing it ', () => {
      handleError({ status: statusStub }, { id: 1, message: 'fake_error' });
      expect(statusStub).to.have.been.calledWith(400);
      expect(jsonStub).to.have.been.calledWith({ id: 1, message: 'fake_error' });
    });
  })
})
